...
def main():
    ...
    with open(os.path.join(data_path, 'category_count_sorted.json'), 'r', encoding='utf-8') as f:
        category = json.load(f)

    count_threshold = 6
    category = {k: v for k, v in category.items() if v >= count_threshold}
    ...
    for file in tqdm(data_list):
        category_info = {k: {'id':[], 'embeddings':[0]*1500} for k in category.keys()}
            ...
            for i in range(df.height):
                ...
                for _category in _doc_category:
                    if _category in category_info.keys():
                        ...
                        category_info[_category]['embeddings'] = list(map(add, category_info[_category]['embeddings'], df['embeddings'][i].to_list()))
                    else:
                        category_info[_category] = {'id': [df['id'][i]], 'embeddings': df['embeddings'][i].to_list()}
            del df
    
    print(f"Pruned {len({k:v for k,v in category_info.items() if len(v['id']) ==0})} categories")
    category_info = {k:v for k, v in category_info.items() if len(v['id']) != 0}
    
    # Calculate the centroid
    for _category in category_info.keys():
        if len(category_info[_category]['id']) == 0:
            print(f'Category {_category} has no document')
        category_info[_category]['centroid'] = list(map(lambda x: x/(len(category_info[_category]['id'])+1), category_info[_category]['embeddings']))
    
    # Remvove embeddings key and value for each category
    for _category in category_info.keys():
        del category_info[_category]['embeddings']
    
    # Save the category info
    with open(os.path.join(category_path, 'category_info_centroid.json'), 'w') as f:
